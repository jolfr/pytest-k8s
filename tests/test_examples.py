...
@pytest.mark.parametrize("k8s_cluster", [
    {"name": "module-scoped-cluster", "scope": "module"}
], indirect=True)
...
@pytest.mark.parametrize("k8s_cluster", [
    {"name": "class-scoped-cluster", "scope": "class"}
], indirect=True)
...
def test_client_with_session_scope(k8s_cluster, k8s_client):
    ...
    default_ns = next(
        (ns for ns in namespaces.items if ns.metadata.name == "default"),
        None
    )
...
def test_multiple_api_clients(k8s_client):
    ...
    namespaces = core_v1.list_namespace()
    assert len(namespaces.items) >= 4  # default, kube-system, kube-public, kube-node-lease
...
def test_bulk_resource_operations(k8s_cluster, k8s_client):
    ...
    try:
        ...
        cm_list = core_v1.list_namespaced_config_map(namespace=namespace)
        bulk_cms = [cm for cm in cm_list.items if cm.metadata.name.startswith("bulk-config-")]
...
def test_complete_application_stack(k8s_cluster, k8s_client):
    ...
    try:
        ...
        secret = client.V1Secret(
            ...
            data={
                "db_password": base64.b64encode(b"secretpass").decode("utf-8")
            }
        )
                    ...
                    spec=client.V1PodSpec(
                        containers=[
                            client.V1Container(
                                ...
                                env=[
                                    client.V1EnvVar(name="POSTGRES_PASSWORD", value="secretpass"),
        ...
        app_deployment = client.V1Deployment(
            ...
            spec=client.V1DeploymentSpec(
                replicas=2,
                selector=client.V1LabelSelector(match_labels={"app": f"{app_name}-app"}),
                template=client.V1PodTemplateSpec(
        ...
        app_endpoints = core_v1.read_namespaced_endpoints(f"{app_name}-service", namespace)
        
        print(f"✓ Database service has {len(db_endpoints.subsets or [])} endpoint subsets")
        print(f"✓ Application service has {len(app_endpoints.subsets or [])} endpoint subsets")
