...
class TestKubernetesClient:
    ...
    def test_repr_representation(self):
        ...
        with patch('kubernetes.config.load_kube_config'):
            client = KubernetesClient(mock_cluster)
            assert repr(client) == "KubernetesClient(cluster=test-cluster, kubeconfig=/tmp/kubeconfig)"
...
class TestClientFixtureIntegration:
    ...
    def test_k8s_client_fixture_basic(self, mock_load_config, mock_cluster_class, k8s_client):
    ...
    @patch('pytest_k8s.fixtures.k8s_cluster.KindCluster')
    @patch('kubernetes.config.load_kube_config')
    def test_k8s_client_with_explicit_cluster(self, mock_load_config, mock_cluster_class, k8s_cluster, k8s_client):
